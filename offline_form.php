...
<head>
    ...
    <title>Offline Appointment Form</title>
...
<body>
    ...
    <div class="container">
        ...
        $sql = "SELECT patient_gender, patient_age, patient_marry, patient_name, patient_mobile FROM patient WHERE patient_id = ?";
        if ($stmt = mysqli_prepare($conn, $sql)) {  //line 62
            mysqli_stmt_bind_param($stmt, "i", $patient_id);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_bind_result($stmt, $patient_gender, $patient_age, $patient_marry, $patient_name, $patient_mobile);
        ...
        if (isset($_POST["submit"])) {
            ...
            $patient_mobile = isset($_POST["patient_mobile"]) ? htmlspecialchars($_POST["patient_mobile"]) : '';
            $doctor_id = isset($_POST["doctor_id"]) ? htmlspecialchars($_POST["doctor_id"]) : '';

            // Insert the patient data into the Anonymous_Form table
            if (!empty($patient_id)) {
                $sql = "INSERT INTO offline_Form (patient_id, patient_gender, patient_age, patient_marry, patient_message, patient_name, patient_mobile, doctor_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_stmt_init($conn);
                if (mysqli_stmt_prepare($stmt, $sql)) {
                    mysqli_stmt_bind_param($stmt, "issssssi", $patient_id, $patient_gender, $patient_age, $patient_marry, $patient_message, $patient_name, $patient_mobile, $doctor_id);
                ...
                if ($update_stmt = mysqli_prepare($conn, $update_sql)) {    //line 98
                    mysqli_stmt_bind_param($update_stmt, "sssssi", $patient_gender, $patient_age, $patient_marry, $patient_name, $patient_mobile, $patient_id);
            ...
            <div class="col-md-6 col-md-offset-3">
                <div class="panel panel-default" style="margin-top: 20px;">
                    <div class="panel-heading">
                        <strong>Offline Appointment Form</strong>
                    </div>
                    <div class="panel-body">
                        <form action="offline_form.php" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="patient_name" class="form-control" placeholder="Full Name" value="<?php echo htmlspecialchars($patient_name); ?>" required="">
                            </div>

                            <div class="form-group">
                                <label>Enter Your Mobile No.</label>
                                <input type="tel" name="patient_mobile" class="form-control" placeholder="Enter your 11 digit mobile no." value="<?php echo htmlspecialchars($patient_mobile); ?>" required="">
                            </div>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_solution']) && isset($_GET['form_id'])) {
    ...
    $update_sql = "UPDATE Anonymous_Form SET anonymous_solution = ? WHERE anonymous_form_id = ? AND doctor_id = ?";
    ...
    if ($update_stmt->execute()) {
        ...
        header("Location: view_anonymous_form.php?form_id=$form_id&solution_submitted=true");
        exit;
    } else {
        $error_message = "Failed to submit solution. Please try again.";
    }
    $update_stmt->close();
}
...
if (isset($_GET['form_id'])) {
    ...
    $sql = "SELECT * FROM Anonymous_Form WHERE anonymous_form_id = ? AND doctor_id = ?";
...
<head>
    ...
    <title>View Anonymous Form</title>
    <style>
        .form-container {
...
<body>
    ...
    <div class="container">
        <div class="form-container">
            ...
            if (isset($form)) {
                ...
                <div class="form-header">
                    <h3>Anonymous Form ID: ' . htmlspecialchars($form['anonymous_form_id']) . '</h3>
                </div>
                ...
                if ($form['anonymous_solution'] === 'Not Answered Yet!') {
                    // Display the solution form if not already solved
                    echo '
                    <div class="solution-form">
                        <form method="post" action="">
                            <div class="form-group">
                                <label for="solution">Submit Your Solution:</label>
                                <textarea class="form-control" id="solution" name="solution" rows="5" required></textarea>
                            </div>
                            <button type="submit" name="submit_solution" class="submit-btn">Submit Solution</button>
                        </form>
                    </div>';
                } else {
                    echo '<p class="text-center">Solution has already been submitted for this form.</p>';
                }
            } else {
        ...
        <div class="back-btn">
            <a href="doctor_anonymous_form_unread.php">Back to Unread Forms</a>
        </div>
    </div>
